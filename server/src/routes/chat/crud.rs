...
async fn get_messages<'a>(
    ...
    let messages = sqlx::query!(r#"
            SELECT m.*,
                ...
                er.initial_location as "initial_location: LatLong",
                er.desired_location_name,
                er.request_content,
                er.approved_by,
                er.approved_at,
                er.came_back_at,
    ...
    for msg in messages {
        ...
        if let Some(content) = msg.content {
            ...
            if let None = msg.request_content {
                return ApiResult::Code(StatusCode::INTERNAL_SERVER_ERROR);
            }
            if let None = msg.approved_at {
                return ApiResult::Code(StatusCode::INTERNAL_SERVER_ERROR);
            }
            if let None = msg.came_back_at {
                return ApiResult::Code(StatusCode::INTERNAL_SERVER_ERROR);
            }
            ...
            result.push(GetChatMessageDto {
                ...
                message_kind: ChatMessageKind::ExitRequest(ChatExitRequest {
                    initial_location: msg.initial_location,
                    desired_location_name: msg.desired_location_name,
