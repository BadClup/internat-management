...
pub async fn ws_handler(
    ...
    let resident_id = match header_map.get("resident-id") {
        ...
        None => {
            if matches!(user.role, UserRole::Resident) {
                ...
                Err(ApiResult::Custom(
                    "Missing resident-id header",
    ...
    ApiResult::Ok(ws.on_upgrade(move |socket| handle_kafka_chat_events(socket, resident_id)))
}
...
async fn handle_kafka_chat_events(mut ws: WebSocket, resident_id: u32) {
    ...
    while let Ok(msg) = kafka_consumer.recv().await {
        ...
        if msg.resident_id != resident_id as i32 {
            _ = ws_send_status_msg(
                &mut ws,
                "Cannot send a message to someone else chat",
                StatusCode::BAD_REQUEST,
            )
            .await;
        }
...
pub(crate) async fn ws_send_status_msg<T>(
    ws: &mut WebSocket,
    msg: T,
...
pub async fn ws_send_internal_error<T>(ws: &mut WebSocket, msg: T) -> Result<(), axum_core::Error>
where
    T: Into<String>,
{
    let msg = msg.into();
    let msg = WsMessage::Message(web_sockets::StatusMessage::from((
        msg,
